...
import { streamChat, type GraphStats } from '../api/chat'
...
const sendMessage = async (): Promise<void> => {
  ...
  sseConnection?.close()
  sseConnection = streamChat({
    chatId: currentChatId.value,
    message: content,
    graphStats: stats,
    onDelta: async (deltaText: string) => {
      const qaItem = qaList.value.find((item) => item.qaId === qaId)
      if (qaItem) {
        qaItem.answer += deltaText
      }
      await scrollChatToBottom()
    },
    onDone: async () => {
    ...
    onError: async (error: Error) => {
      const qaItem = qaList.value.find((item) => item.qaId === qaId)
      if (qaItem) {
        qaItem.answer = qaItem.answer || `请求失败：${error.message || 'SSE 请求异常'}`
      }
      isReplying.value = false
      await scrollChatToBottom()
    },
...
<template>
  <aside class="chat-panel">
    ...
    <div ref="chatBodyRef" class="chat-body">
      <div class="qa-intro">
        你好，我是流程图 AI 助手。请输入问题，我会以问答形式返回结果。
      </div>
      <div
        v-for="qa in qaList"
        :key="qa.qaId"
